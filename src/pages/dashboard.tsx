...
import { getInterests } from 'apiSdk/interests';
import { InterestInterface } from 'interfaces/interest';

import { InterestListPage as ShowCaseList } from 'pages/interests';
...
function DashboardPage() {
  ...
  const interestStat = useSWR<PaginatedInterface<InterestInterface>>(
    () => `/interests`,
    () => getInterests({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Interests',
      result: interestStat?.data?.totalCount,
      isLoading: interestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`interests`} passHref legacyBehavior>
                  <Button
