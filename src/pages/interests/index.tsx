...
type ColumnType = ColumnDef<InterestInterface, unknown>;

interface InterestListPageProps {
...
export function InterestListPage(props: InterestListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InterestInterface>>(
    () => `/interests?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/interests/view/${record.id}`} passHref legacyBehavior>
                <Button
