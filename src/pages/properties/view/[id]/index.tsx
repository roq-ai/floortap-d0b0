...
import { InspectionListPage } from 'pages/inspections';
import { InterestListPage } from 'pages/interests';
...
function PropertyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PropertyInterface>(
    ...
    () =>
      getPropertyById(id, {
        relations: ['user_property_seller_idTouser', 'user_property_buyer_idTouser', 'user_property_agent_idTouser'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Property Seller Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_property_seller_idTouser?.id}`}>
                        {data?.user_property_seller_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Property Buyer Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_property_buyer_idTouser?.id}`}>
                        {data?.user_property_buyer_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Property Agent Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_property_agent_idTouser?.id}`}>
                        {data?.user_property_agent_idTouser?.email}
                      </Link>
                    }
                  />
                )}
