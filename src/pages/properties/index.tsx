...
export function PropertyListPage(props: PropertyListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProperties({
        relations: [
          'user_property_seller_idTouser',
          'user_property_buyer_idTouser',
          'user_property_agent_idTouser',
          'inspection.count',
          'interest.count',
          'offer.count',
        ],
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'location.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_property_seller_idTouser',
          header: 'User Property Seller Id Touser',
          accessorKey: 'user_property_seller_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_property_seller_idTouser?.id}`}
            >
              {record.user_property_seller_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_property_buyer_idTouser',
          header: 'User Property Buyer Id Touser',
          accessorKey: 'user_property_buyer_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_property_buyer_idTouser?.id}`}
            >
              {record.user_property_buyer_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_property_agent_idTouser',
          header: 'User Property Agent Id Touser',
          accessorKey: 'user_property_agent_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_property_agent_idTouser?.id}`}
            >
              {record.user_property_agent_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('inspection', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'inspection',
          header: 'Inspection',
          accessorKey: 'inspection',
          cell: ({ row: { original: record } }: any) => record?._count?.inspection,
        }
      : null,
    hasAccess('interest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.interest,
        }
      : null,
